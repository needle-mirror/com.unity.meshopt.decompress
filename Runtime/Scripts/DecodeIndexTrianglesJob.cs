...
namespace Meshoptimizer {
    ...
    unsafe struct DecodeIndexTrianglesJob : IJob {
        ...
        NativeArray<uint> InitFifo(uint length) {
        ...
        void PushEdgeFifo(ref NativeArray<uint> fifo, uint a, uint b, ref uint offset) {
	        fifo[(int)offset] = a;
	        fifo[(int)(offset|0x10)] = b;
	        offset = (offset + 1) & 15;
        }

        void PushVertexFifo(ref NativeArray<uint> fifo, uint v, ref uint offset, uint cond = 1) {
	        fifo[(int)offset] = v;
